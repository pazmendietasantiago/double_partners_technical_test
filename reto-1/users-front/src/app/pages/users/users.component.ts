...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  getPersons(page: number): void {
    const subscription = this.personService
      ...
      .subscribe((response: any) => {
        this.persons = response.map((props: any) => {
  ...
  getUsers(): void {
    const subscription = this.userService
      .getUsers()
      .subscribe((response: any) => {
        this.users = response.map((props: any) => {
  ...
  savePerson(): void {
    ...
    this.person.tipoIdentificacion = documentType.value;
  ...
  saveUser(): void {
    this.user.usuario1 = this.user.usuario ?? emptyString;
